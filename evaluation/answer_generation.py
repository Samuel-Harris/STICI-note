...
def main():
    ...
    multi_passage_df.rename(columns={"answer": "expected_answer"})
    single_passage_df.rename(columns={"answer": "expected_answer"})
    ...
    test_pipeline_df.to_csv("../data/test_pipelines.csv")
...
def run_test_on_dataset(documents_df: pd.DataFrame, dataset: pd.DataFrame, test_pipeline: TestPipeline,
                        dataset_name: str, test_pipeline_i: int):
    ...
    dataset.to_csv(f"../data/{dataset_name}_pipeline_{test_pipeline_i}_responses.csv")
